<nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b-0 border-gray-200 dark:border-gray-600">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            @if (Route::has('login') && Auth::check())
                ...
                @if (Auth::user()->hasRole('admin'))
                    <a href="{{ url('/admin/dashboard') }}" <button type="button"
                        class="text-white bg-blue-600 hover:bg-orange-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-orange-400 dark:focus:ring-blue-800">Dashboard
                        </button>
                    </a>
                @endif
                @if (Auth::user()->hasRole('candidate'))
                    <a href="{{ url('/candidate/dashboard') }}" <button type="button"
                        class="text-white bg-blue-600 hover:bg-orange-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-orange-400 dark:focus:ring-blue-800">Dashboard
                        </button>
                    </a>
                @endif
                @if (Auth::user()->hasRole('investor'))
                    <a href="{{ url('/investor/dashboard') }}" <button type="button"
                        class="text-white bg-blue-600 hover:bg-orange-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-orange-400 dark:focus:ring-blue-800">Dashboard
                        </button>
                    </a>
                @endif
        ...
        <div class="-ml-20 items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
            <ul
                ...
                <li>
                    <a href="#"
                        class="block py-2 px-3 text-gray-900 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">FAQS</a>
                </li>
            </ul>
        </div>
    </div>
